...
def load_dataset(transcripts_path):
    ...
    with open(transcripts_path, encoding='utf-8') as f:
        for idx, line in enumerate(f.readlines()):
            try:
                audio_path, korean_transcript, transcript = line.strip().split('\t')
...
def _parse_transcript(transcript, sos_id, eos_id):
    tokens = transcript.strip().split(' ')
    transcript_ids = [int(sos_id)]
    for token in tokens:
        try:
            transcript_ids.append(int(token))
        except ValueError:
            print("Original transcript:", transcript)
            print(f"Warning: Skipping invalid token '{token}' in transcript.")
    transcript_ids.append(int(eos_id))
    return torch.tensor(transcript_ids)
